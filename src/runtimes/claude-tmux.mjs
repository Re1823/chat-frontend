...
export function createClaudeTmuxRuntime({config,transport,registry,turnStore,ingress,frameBufferFactory=createFrameBuffer}){
  ...
  const processRaw=async raw=>{
    ...
    if(frame.kind==='turn_error'||(frame.kind==='turn_stop'&&frame.outcome==='failed')){
      ...
      turnStore.finish(turn.runtimeId,turn.turnId,turnEvent.error(turn.turnId,frame.message||frame.reason||'Claude Code 回复失败'));
      return {accepted:true};
    }
  ...
  return {
    ...
    async initialize(){
      ...
      if(reconciliation.state==='missing'&&config.autoCreate){await transport.createSession({sessionName:saved.sessionName,workspace:saved.workspace,command:saved.claudeCommand||config.claudeBinary,args:config.launchArgs||[]});reconciliation=await registry.reconcile()}
      return reconciliation;
    },
    async status(){reconciliation=await registry.reconcile();return reconciliation},
    async preflight(runtimeId){
      if(!config.enabled)throw Object.assign(new Error(config.unsupportedPlatform?'Claude tmux runtime 只支持 Linux/WSL/VPS':'Claude tmux runtime 未启用'),{statusCode:503});
    ...
    async chat({runtimeId,prompt,emit,turnId=randomUUID()}){
    ...
    async stop({runtimeId,turnId}){
      ...
      await transport.interrupt(runtime.sessionName);
